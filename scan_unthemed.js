...
const spellScanPath = path.join(__dirname, 'spell_scan_output.json')
const spellScan = JSON.parse(fs.readFileSync(spellScanPath, 'utf8'))
const spells = spellScan.spells

function classifySpellByName(name, hasTome) {
  ...
  if (/\bflesh\b/.test(lower) || /flesh/.test(lower)) return 'Armor'
...
const unthemed = spells.filter(s => {
  const theme = classifySpellByName(s.name, !!s.tomeFormId)
  return theme === 'Other'
})

const outPath = path.join(__dirname, 'unthemed_spells.txt')
fs.writeFileSync(outPath, unthemed.map(s => s.name).join('\n') + '\n', 'utf8')

console.log(`Total spells: ${spells.length}`)
console.log(`Unthemed spells (Other): ${unthemed.length}`)
