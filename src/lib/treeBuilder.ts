...
function classifySpellByName(name: string, hasTome?: boolean): string {
  ...
  if (/\b(fire|flames?|burn|incinerate|firebolt|fireball|flamecloak|cloak|wall\s+of\s+flames?|scorch\w*|flamestrike|hellfire|firebomb|flare|dull\s+embers|inferno|apocalypse|carbonize|pyroclasm|immolate|fiery\s+grasp|searing\s+grasp|firebloom|flameheart|incendiary\s+flow|infernal\s+mist|balefire|molten\w*|arclight\w*|third\s+era\s+apocalypse|ash\s+cloud|burning\s+touch|ghostflame|ash\w*|illusory\s+pyre|chaos\s+furnace|chaos\s+forge|lustmord|salamander\s+scales)\b/.test(lower)) return 'Fire'
  if (/\b(frost|ice|cold|freeze|frozen|icy|frostbite|icy\s+spear|wall\s+of\s+frost|snow|sleet|hail|hailstone|blizzard|fimbul|midnight\s+snow|vulom\'?s\s+blizzard|tsunice\s+storm|winter|unbounded\s+freezing|frostbolt|cryoblast|hoarfrost|chilling\s+touch|freezing\s+grasp|frostburst|icicle|arctic\s+fortress|boreal\s+breath|boreal\s+touch|frostblossom|enchanted\s+snowball|hailthorns)\b/.test(lower)) return 'Frost'
  if (/\b(shock|lightning|thunder|thunderbolt|thunderburst|storm|sparks?|wall\s+of\s+lightning|zap|unbounded\s+storms|teslasphere|tonitrus|scattershock|thundercrack|crackle|electrocute|electrify|stormblast|crackling\s+static|howling\s+blast|thundering\s+grasp|thundering\s+hooves|concussive\s+blast|electrosphere|witch\s+bolt|static\s+touch|overcharge|shocking\s+touch|elemental\s+bolt)\b/.test(lower)) return 'Shock'
  if (/\b(sound|sonic|noise|echo|reverberate|resonance|frequency|pitch|tone|melody|harmony|chord|octave|note|beat|rhythm|pulse|wave|vibration|thunder|thunderbolt|thunderburst|crackle|howl|roar|scream|wail|shriek|blast|burst|shockwave|concussive|reverbra|mega\s+reverbra|novasphere|resona|amplifier|muffler|streak|tonitrus|maelstrom|supernova)\b/.test(lower)) return 'Sound'
  ...
  if (/\b(heal|restore|cure|regeneration|healing|salvation|close\s+(?:greater\s+)?wounds|rejuvenate|rejuvenating\s+touch|revitalizing\s+growth|cleanse|azure\s+reconstruction|resurgence|intervention|valkyrie\'?s\s+embrace)\b/.test(lower)) return 'Healing'
  ...
  if (/\b(dark|void|shadow|umbral|gloaming|daumbra|eldritch|arcane|mystic|ether|cosmic|mundus\s+veil|fluttering\s+shadows|twilight\s+sickle|darkness|gather\s+shadows|infernal\s+mist|shadowbond|evil\s+twin|mirror\s+entity|wyrd|riftveil\s+tendrils|the\s+celestial\s+volume\'?s|nebula\'?s\s+finale|abyssal\s+oversight|apocryphal\s+gates|abyssal\s+ink\s+bolt|time\s+break|withershins|lodesphere|backlash|attunement|bolide|tharn\'?s\s+prison|malviser\'?s\s+gauntlet|blasting\s+rune|exile|elemental\s+bolt|fade|nightblade|nightfall)\b/.test(lower)) return 'Arcane'
  if (/\b(astral|celestial|star|stardust|eclipse|planar|radiant|oblivion|night\s+gate|magicka|lifeforce|cosmic|void\s+walker|plane|dimensional|rift\s+bolt|rift|cosmos|universe|galaxy|nebula|constellation|zodiac|orbit|planet|moon|solar|lunar|solaris|lunarius|astral\s+plane|celestial\s+plane|ethereal|aether|aetherius|spirit\s+world|heaven|heavenly|divine\s+light|holy\s+light|sacred\s+light|blinding\s+light|radiance|luminescence|incandescence|effulgence|refulgence|resplendence|splendor|majesty|magnificence|grandeur|glory|glorious|magnificent|sublime|exalted|elevated|lofty|high|higher|highest|supreme|ultimate|final|last|end|omega|alpha|beginning|origin|source|fountain|wellspring|font|spring|head|source|root|radix|radical|fundamental|basic|elementary|primary|first|original|initial|inaugural|maiden|virgin|fresh|new|novel|innovative|creative|inventive|imaginative|visionary|prophetic|prophecy|oracle|augur|auspex|haruspex|soothsayer|seer|visionary|dreamer|prophet|messiah|savior|redeemer|deliverer|rescuer|savior|protector|defender|guardian|warden|keeper|curator|steward|caretaker|attendant|servant|minister|chaplain|pastor|priest|priestess|cleric|monk|nun|abbot|abbess|bishop|archbishop|cardinal|pope|pontiff|patriarch|matriarch|elder|ancients| progenitor|ancestor|forefather|foremother|parent|generator|genitor|genetrix|creator|maker|artificer|craftsman|craftswoman|artisan|artist|composer|poet|bard|skald|minstrel|troubadour|trouvere| Minnesinger|Meistersinger|singer|vocalist|musician|instrumentalist|player|performer|actor|actress|thespian|player|participant|contributor|donor|giver|giver|presenter|bestower|conferrer|grantor|donor|benefactor|patron|sponsor|backer|investor|banker|financier|moneylender|usurer|loan shark|shylock| Shylock|capitalist|entrepreneur|businessman|businesswoman|merchant|trader|dealer|vendor|seller|salesperson|salesman|saleswoman|cashier|clerk|receptionist|secretary|assistant|aide|adjutant|helper|assistor|supporter|advocate|champion|defender|protector|guardian|savior|rescuer|deliverer|liberator|emancipator|freedman|freedwoman|free|liberty|freedom|independence|autonomy|sovereignty|self-determination|self-rule|self-governance|self-mastery|self-control|self-restraint|self-discipline|self-denial|self-sacrifice|self-abnegation|self-renunciation|self-annihilation|self-destruction|suicide|self-murder|homicide|murder|kill|slay|execute|assassinate|eliminate|exterminate|annihilate|destroy|devastate|ravage|wreck|ruin|spoil|corrupt|pollute|contaminate|infect|taint|defile|desecrate|profane|violate|infringe|transgress|offend|sin|transgression|iniquity|wickedness|evil|vice|depravity|corruption|debasement|degradation|depravity|vice|crime|felony|misdemeanor|offense|infraction|violation|breach|infringement|contravention|noncompliance|disobedience|defiance|rebellion|insurrection|mutiny|revolt|revolution|coup|overthrow|downfall|ruin|doom|fate|destiny|fortune|luck|chance|probability|statistical|mathematical|logical|rational|reasonable|sensible|sound|valid|strong|powerful|mighty|potent|effective|efficient|productive|fruitful|fertile|prolific|abundant|ample|plentiful|copious|liberal|generous|bountiful|bounteous|lavish|profuse|abundant|affluent|wealthy|rich|prosperous|thriving|flourishing|blooming|blossoming|budding|growing|developing|evolving|progressing|advancing|improving|better|best|optimal|optimum|ideal|perfect|complete|finished|done|concluded|ended|terminated|stopped|halted|ceased|discontinued|abandoned|deserted|forsaken|neglected|ignored|overlooked|forgotten|lost|missing|gone|vanished|disappeared|absent|away|out)\b/.test(lower)) return 'Arcane'
  if (/\b(astral|celestial|star|stardust|eclipse|planar|radiant|oblivion|night\s+gate|magicka|lifeforce|cosmic|void\s+walker|plane|dimensional|rift\s+bolt|rift|cosmos|universe|galaxy|nebula|constellation|zodiac|orbit|planet|moon|solar|lunar|solaris|lunarius|astral\s+plane|celestial\s+plane|ethereal|aether|aetherius|spirit\s+world|heaven|heavenly|divine\s+light|holy\s+light|sacred\s+light|blinding\s+light|radiance|luminescence|incandescence|effulgence|refulgence|resplendence|splendor|majesty|magnificence|grandeur|glory|glorious|magnificent|sublime|exalted|elevated|lofty|high|higher|highest|supreme|ultimate|final|last|end|omega|alpha|beginning|origin|source|fountain|wellspring|font|spring|head|source|root|radix|radical|fundamental|basic|elementary|primary|first|original|initial|inaugural|maiden|virgin|fresh|new|novel|innovative|creative|inventive|imaginative|visionary|prophetic|prophecy|oracle|augur|auspex|haruspex|soothsayer|seer|visionary|dreamer|prophet|messiah|savior|redeemer|deliverer|rescuer|savior|protector|defender|guardian|warden|keeper|curator|steward|caretaker|attendant|servant|minister|chaplain|pastor|priest|priestess|cleric|monk|nun|abbot|abbess|bishop|archbishop|cardinal|pope|pontiff|patriarch|matriarch|elder|ancients| progenitor|ancestor|forefather|foremother|parent|generator|genitor|genetrix|creator|maker|artificer|craftsman|craftswoman|artisan|artist|composer|poet|bard|skald|minstrel|troubadour|trouvere| Minnesinger|Meistersinger|singer|vocalist|musician|instrumentalist|player|performer|actor|actress|thespian|player|participant|contributor|donor|giver|giver|presenter|bestower|conferrer|grantor|donor|benefactor|patron|sponsor|backer|investor|banker|financier|moneylender|usurer|loan shark|shylock| Shylock|capitalist|entrepreneur|businessman|businesswoman|merchant|trader|dealer|vendor|seller|salesperson|salesman|saleswoman|cashier|clerk|receptionist|secretary|assistant|aide|adjutant|helper|assistor|supporter|advocate|champion|defender|protector|guardian|savior|rescuer|deliverer|liberator|emancipator|freedman|freedwoman|free|liberty|freedom|independence|autonomy|sovereignty|self-determination|self-rule|self-governance|self-mastery|self-control|self-restraint|self-discipline|self-denial|self-sacrifice|self-abnegation|self-renunciation|self-annihilation|self-destruction|suicide|self-murder|homicide|murder|kill|slay|execute|assassinate|eliminate|exterminate|annihilate|destroy|devastate|ravage|wreck|ruin|spoil|corrupt|pollute|contaminate|infect|taint|defile|desecrate|profane|violate|infringe|transgress|offend|sin|transgression|iniquity|wickedness|evil|vice|depravity|corruption|debasement|degradation|depravity|vice|crime|felony|misdemeanor|offense|infraction|violation|breach|infringement|contravention|noncompliance|disobedience|defiance|rebellion|insurrection|mutiny|revolt|revolution|coup|overthrow|downfall|ruin|doom|fate|destiny|fortune|luck|chance|probability|statistical|mathematical|logical|rational|reasonable|sensible|sound|valid|strong|powerful|mighty|potent|effective|efficient|productive|fruitful|fertile|prolific|abundant|ample|plentiful|copious|liberal|generous|bountiful|bounteous|lavish|profuse|abundant|affluent|wealthy|rich|prosperous|thriving|flourishing|blooming|blossoming|budding|growing|developing|evolving|progressing|advancing|improving|better|best|optimal|optimum|ideal|perfect|complete|finished|done|concluded|ended|terminated|stopped|halted|ceased|discontinued|abandoned|deserted|forsaken|neglected|ignored|overlooked|forgotten|lost|missing|gone|vanished|disappeared|absent|away|out)\b/.test(lower)) return 'Arcane'
  if (/\b(astral|celestial|star|stardust|eclipse|planar|radiant|oblivion|night\s+gate|magicka|lifeforce|cosmic|void\s+walker|plane|dimensional|rift\s+bolt|rift|cosmos|universe|galaxy|nebula|constellation|zodiac|orbit|planet|moon|solar|lunar|ethereal|aether|aetherius|heaven|heavenly|blinding\s+light|radiance|luminescence|splendor|majesty|glory|sublime|exalted|lofty|supreme|omega|alpha|origin|source|fountain|fundamental|primary|original|visionary|prophetic|oracle|augur|seer|prophet|messiah|savior|redeemer|deliverer|rescuer|protector|defender|guardian|warden|keeper|curator|steward|creator|maker|artificer|composer|poet|bard|performer|player|contributor|donor|giver|presenter|bestower|conferrer|grantor|benefactor|patron|free|liberty|freedom|independence|autonomy|sovereignty|true\s+path|race\s+against\s+time|milestones|biorhythm|power\s+of\s+the\s+master|channel\s+energies|perilous\s+path|translimination|meridian\s+rune|pride\s+of\s+hirstaang|fenrik\'?s\s+welcome|ysgrim\'?s\s+thunderbread|chase\s+the\s+horizon)\b/.test(lower)) return 'Astral'
  if (/\b(holy|sacred|divine|hallowed|blessed|sanctified|arkay|stendarr|mara|auriel|wrath\s+of\s+the\s+heavens|absolution|royal\s+absolution|penance|penance\s+aura\s+-\s+fanning|penance\s+aura\s+-\s+shielding|aedric\s+scepter|meridia\'?s\s+wrath|daybreak|radiant\s+sunlight\s+rune|wall\s+of\s+sunlight|eye\s+of\s+the\s+all-maker|augur\s+of\s+aetherius|desperate\s+prayer|godform|crown\'?s\s+verdict|voice\s+of\s+judgment|voice\s+of\s+the\s+king|king\'?s\s+cross|reclamations\s+intervention|scion\'?s\s+embrace|rite\s+of\s+the\s+old\s+gods|salvation|celestial\s+fortitude|aid|radiant\s+oppression|bestow\s+gift)\b/.test(lower)) return 'Holy'
  if (/\b(royal|crown|king|monarch|throne|scepter|kingdom|empire|dynasty|regent|sovereign|ruler|crown\'?s|king\'?s|monarch\'?s|royal\s+decree|bolt\s+of\s+the\s+monarch|aura\s+of\s+the\s+monarch|hand\s+of\s+the\s+king|echo\s+of\s+the\s+throne|scepter\s+of\s+highreach|crown\'?s\s+cataclysm|crown\'?s\s+vortex)\b/.test(lower)) return 'Royal'
  ...
  if (/\b(transmogrify|polymorph|shapeshift|transform|disguise|wildshape|beast\s+form|werewolf|vampire\s+lord|open\s+\w+\s+lock|unlock\s+container|detonate\s+lock|unlock|fabricate\s+object)\b/.test(lower)) return 'Transform'
  if (/\b(telekinesis|levitate|fly\s+with\s+me|float|aard\s+-\s+telekinetic\s+strike|concentrated\s+air\s+blades)\b/.test(lower)) return 'Telekinesis'
  if (/\b(fortify|ocato|boost|blessing|inspire|invigorate|vigor|endurance|strength|might|supernatural\s+reflexes|endowment\s+of\s+wings|haste|speed|swift\s+swim|longstride|wind\s+running|windwalker|rally|battletide|rampage|predator|predator\s+vision|swift\s+chase|enduring\s+horde|blitzkrieg|rite\s+of\s+wings|energize|vitalisation|wildness|ferality|prismatic\s+skin|slowfall|pack\s+mule|ease\s+burden|heel\s+refresh|featherlight|springheel|acceleration\s+rune|horny)\b/.test(lower)) return 'Buff'
  if (/\b(equilibrium|balance|symmetry)\b/.test(lower)) return 'Utility'
  if (/\b(waterbreathing|ease\s+burden|ysgrim\'?s\s+thunderbread|heel\s+refresh|xdeploy\s+pwnyx|alarm|prepare\s+for\s+adventure|loot\s+magnet|tumble\s+magnet|spell\s+twine|encumber|burden|burden\s+rune|leap|springheel|featherlight|quietude)\b/.test(lower)) return 'Utility'
  if (/\b(waterbreathing|ease\s+burden|ysgrim\'?s\s+thunderbread|heel\s+refresh|xdeploy\s+pwnyx|alarm|prepare\s+for\s+adventure|loot\s+magnet|tumble\s+magnet|spell\s+twine|encumber|burden|burden\s+rune|leap|springheel|featherlight|quietude)\b/.test(lower)) return 'Utility'
  ...
  if (/\b(stomp|knockdown|bash|slam|push|pull|force|gravity|quake|earthquake|tremor|impact|sunder|shatter|crush|bombardment|fracture|tremble|thundering\s+hooves|disintegrate\s+weapon|destroy\s+construct|weight\s+of\s+the\s+world|last\s+word|hurl\s+into\s+oblivion|bolide|chakram\s+shields|diamond\s+splash|grand\s+cross)\b/.test(lower)) return 'Force'
